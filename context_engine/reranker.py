...
def _get_chunk_embedding(chunk: Dict[str, Any], driver=None) -> Optional[List[float]]:
    """
    Get embedding for a chunk.

    First tries the chunk dict, then falls back to database lookup.
    """
    # Check if embedding is in chunk
    if chunk.get("embedding"):
        return chunk["embedding"]

    # Try database lookup if driver provided
    if driver is None:
        return None
    ...
    try:
        ...
        with driver.session(database=NEO4J_DB) as s:
            ...
            """, cid=chunk["chunk_id"]).single()

            if result and result["embedding"]:
                return list(result["embedding"])
    except Exception as e:
        logger.debug(f"Failed to fetch embedding for chunk {chunk['chunk_id'][:8]}: {e}")
...
def rerank_by_embedding(query_embedding: List[float],
    ...
    for chunk in chunks:
        chunk_emb = _get_chunk_embedding(chunk, driver)
        if chunk_emb:
...
def mmr_rerank(query_embedding: List[float],
    ...
    for chunk in chunks:
        emb = _get_chunk_embedding(chunk, driver)
