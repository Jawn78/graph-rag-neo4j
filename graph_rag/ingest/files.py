...
def _chunk_text(text: str, target_chars: int = CHUNK_TARGET_CHARS,
    ...
    # Hard-split any "sentence" longer than the target (common in extracted
    # PDFs with missing punctuation) so a single run-on line can't produce a
    # chunk many times the intended size.
    pieces: List[str] = []
    for s in sents:
        while len(s) > target_chars:
            pieces.append(s[:target_chars])
            s = s[target_chars:]
        if s:
            pieces.append(s)
    ...
    with ProcessPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                # Stable identity: derive doc_id from the normalized path so
                # re-ingesting an edited file updates the same Document instead
                # of creating a duplicate. Track content changes separately.
                norm_path = os.path.normcase(os.path.abspath(path)).replace("\\", "/")
                did = _sha1(norm_path)
                meta["content_sha1"] = _sha1(text)
                docs.append({
                    "doc_id": did,
                    "title": title,
                    "text": text,
...
def _build_chunks(docs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for d in docs:
        # Reset per document — a heading must never bleed into the next file
        current_heading = ""
...
def _embed_in_place(chunks: List[Dict[str, Any]]) -> None:
    ...
    Must not be called from within a running event loop: embeddings have to be
    complete before the chunks are upserted, so fire-and-forget scheduling is
    not an option. asyncio.run() raises RuntimeError in that case, which is
...
def ingest_folder(driver: Driver, folder: str) -> Tuple[int, int]:
    ...
    for path_batch in _batched(paths, STREAM_BATCH_SIZE):
        ...
        n_docs, n_chunks = _ingest_doc_batch(driver, docs)
        total_docs += n_docs
        total_chunks += n_chunks

        logger.info(f"[{trace_id}] Batch complete: {n_docs} docs, {n_chunks} chunks with vectors")

    logger.info(f"[{trace_id}] Ingestion complete: {total_docs} docs, {total_chunks} chunks")
    return (total_docs, total_chunks)


def _ingest_doc_batch(driver: Driver, docs: List[Dict[str, Any]]) -> Tuple[int, int]:
    """
    Chunk, embed, and upsert a batch of parsed documents, then remove any
    stale chunks left over from a previous version of each document.
    """
    from ..graph.cleanup import delete_stale_chunks
    ...
    # Doc identity is path-based, so a re-ingested document keeps its doc_id;
    # drop chunks from earlier versions that are no longer produced.
    chunks_by_doc: Dict[str, List[str]] = {}
    for c in chunks:
        chunks_by_doc.setdefault(c["doc_id"], []).append(c["chunk_id"])
    for doc_id, keep_ids in chunks_by_doc.items():
        removed = delete_stale_chunks(driver, doc_id, keep_ids)
        if removed:
            logger.info(f"Removed {removed} stale chunks from doc {doc_id[:8]}")
...
def ingest_files(driver: Driver, paths: List[str]) -> Tuple[int, int]:
    ...
    n_docs, n_chunks = _ingest_doc_batch(driver, docs)
    logger.info(f"[{trace_id}] Ingested {n_docs} docs, {n_chunks} chunks with vectors")
