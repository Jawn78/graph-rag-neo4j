...
# Lucene special characters that must be escaped in query terms, otherwise a
# token like "c++" or "and/or" raises a parse error inside Neo4j (which we
# swallow and turn into an empty result set).
_LUCENE_SPECIALS = re.compile(r'([+\-!(){}\[\]^"~*?:\\/]|&&|\|\|)')


def _escape_lucene(term: str) -> str:
    """Escape Lucene query syntax characters in a search term."""
...
def _fts_query_from_question(q: str) -> str:
    ...
    parts = []
    parts += [f'"{_escape_lucene(p)}"' for p in phrases]  # Quoted phrases
    parts += [_escape_lucene(t) for t in toks]  # Individual tokens
    return " OR ".join(parts) if parts else _escape_lucene(q or "")
    ...
    return [
        {
            ...
            "doc_id": r["doc_id"],
            "title": r["title"] or "",
            "_score": 0.0,   # anchor chunks are unscored; rerankers score live
            "metadata": {},
        }
        for r in rows
    ]
...
def _get_llm_response(messages: List[Dict], use_mcp: bool, trace_id: str) -> Tuple[Optional[LLMResponse], Optional[str]]:
    ...
    Order when use_mcp=True:
        1. Agent gateway (MCP_AGENT_URL) — mediates with an MCP server
        2. Default chat client
...
def _embed_query(text: str, trace_id: str) -> Optional[List[float]]:
    """Embed the query text. Returns None on failure (already logged)."""
    try:
    ...
    if include_neighbors:
        ...
        if best:
            ...
            logger.debug(f"[{trace_id}] Anchor: doc={doc_id[:8]}, order={ord0}, "
                         f"got {len(ctx_chunks)} neighbors")

    try:
        hyb = hybrid_search(driver, q_emb, fts_q, top_k=top_k * fetch_multiplier) or []
    except Exception as e:
...
def _finalize_answer(response: Optional[LLMResponse], use_mcp: bool) -> str:
    """Sanitize the LLM answer and annotate MCP fallback if applicable."""
...
def ask(driver, question: str, top_k: int = 6, use_mcp: bool = False) -> Dict[str, Any]:
    ...
    q_emb = _embed_query(question, trace_id)
    if q_emb is None:
        return {"answer": "", "citations": [], "used_chunks": [], "error": "Embedding failed"}

    # 2) Retrieve (FTS anchor + hybrid search)
    ctx_chunks = _retrieve_chunks(driver, question, q_emb, top_k, trace_id=trace_id)

    # 3) Hydrate headings and apply boost
    _hydrate_headings(driver, ctx_chunks)
    ctx_chunks = _apply_heading_boost(question, ctx_chunks)

    # 4) Pack context and build prompt
    ...
    # 5) Get LLM response using fallback chain
...
def ask_with_context(driver, question: str, session_id: Optional[str] = None,
    ...
    q_emb = _embed_query(effective_query, trace_id)
    if q_emb is None:
        return {
            "answer": "",
            "citations": [],
